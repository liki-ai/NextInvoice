...
const translations = {
  sq: {
    ...
    hero: {
      ...
      ctaPrimary: "Fillo falas në web",
      ctaSecondary: "Shiko si funksionon",
      ctaAppStore: "Shkarko në App Store",
    },
    howItWorks: {
      tag: "Rrjedha",
    ...
    ctaSection: {
      ...
      subtitle: "Krijo llogari në web ose shkarko aplikacionin iOS.",
      cta: "Fillo falas",
    },
    footer: {
  ...
  en: {
    ...
    ctaSection: {
      ...
      subtitle: "Create a web account or download the iOS app.",
      cta: "Start for free",
    },
    footer: {
    ...
    <div className="min-h-screen text-brand-ink bg-brand-bg font-sans selection:bg-brand-accent selection:text-brand-ink">
      ...
      <header className="absolute top-0 left-0 right-0 z-50">
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6 md:px-10 md:py-8">
          ...
          <div className="flex items-center gap-3">
            <LangSwitch />
            {user ? (
              <Link to="/app" className="inline-flex items-center rounded-full bg-brand-ink px-5 py-2.5 text-xs font-bold text-white transition hover:bg-brand-dark">
                {t.nav.openApp}
              </Link>
            ) : (
              <>
                <Link to="/login" className="hidden text-sm font-bold text-brand-ink/70 transition hover:text-brand md:inline">
                  {t.nav.login}
                </Link>
                <Link to="/signup" className="inline-flex items-center rounded-full bg-brand-ink px-5 py-2.5 text-xs font-bold text-white transition hover:bg-brand-dark">
                  {t.nav.signup}
                </Link>
              </>
            )}
      ...
      <section id="top" className="relative overflow-hidden hero-atmosphere pt-20">
        ...
        <div className="relative mx-auto grid max-w-7xl grid-cols-1 items-center gap-12 px-6 pt-24 pb-20 md:grid-cols-12 md:gap-16 md:px-10 md:pt-36 md:pb-32">
          <div className="md:col-span-7">
            ...
            >
              {user ? (
                <Link to="/app" className="group inline-flex items-center gap-3 rounded-full bg-brand-ink px-8 py-4 text-sm font-bold text-white shadow-xl shadow-brand-dark/20 transition-all hover:bg-brand-dark hover:-translate-y-1">
                  <span>{t.nav.openApp}</span>
                </Link>
              ) : (
                <Link to="/signup" className="group inline-flex items-center gap-3 rounded-full bg-brand-ink px-8 py-4 text-sm font-bold text-white shadow-xl shadow-brand-dark/20 transition-all hover:bg-brand-dark hover:-translate-y-1">
              ...
              <a href={APP_STORE_URL} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-brand-ink/10 bg-white/40 px-8 py-4 text-sm font-bold text-brand-ink backdrop-blur-md transition-all hover:bg-white">
      ...
      <section className="section-deep relative py-24 md:py-40 overflow-hidden text-white">
        ...
        <div className="relative mx-auto max-w-5xl px-6 text-center md:px-10">
          ...
          >
            <Link to={user ? '/app' : '/signup'} className="inline-flex items-center gap-3 rounded-full bg-brand-accent px-10 py-5 text-sm font-bold text-brand-ink transition-all hover:scale-105 active:scale-95 shadow-xl shadow-black/20">
              {t.ctaSection.cta}
            </Link>
          </motion.div>
        </div>
