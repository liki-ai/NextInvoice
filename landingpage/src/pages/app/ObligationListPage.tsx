...
export function ObligationListPage() {
  const { obligations, invoices, loading, profile, updateObligation } = useAppData()
  const { t } = useI18n()
  ...
  const paid = obligations
    .filter((item) => obligationStatus(item) === 'paid')
    .reduce((sum, item) => sum + (Number(item.amount) || 0), 0)
                      ...
                      <td className="px-5 py-4 text-brand-ink/60">
                        ...
                        <button
                          ...
                          className={`mt-1 inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold ${
                            status === 'paid' ? 'bg-success/10 text-[#2E7D32]' : 'bg-brand/10 text-brand-ink/70'
                          }`}
                        >
